      ...
      VMware_VCloud_SDK_Http_Client_Interface
{
    ...
    {
        if ($this->stringEndsWith($url, '/action/recomposeVApp')) {
            $data=str_replace('<ovf:StartupSection', '<ovf:StartupSection xmlns:ns5="http://www.vmware.com/vcloud/v1.5" ', $data);
        }
        return $this->sendRequest($url,
    ...
    {
        if ($this->stringEndsWith($url, 'virtualHardwareSection/memory') || $this->stringEndsWith($url, 'virtualHardwareSection/cpu')) {
            $data=str_replace('<ns5:Item', '<ns5:Item xmlns:ns5="http://www.vmware.com/vcloud/v1.5" ', $data);
        }
        return $this->sendRequest($url,
