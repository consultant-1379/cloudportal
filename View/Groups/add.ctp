...
<?php
    ...
    echo $this->Form->input('group_dn', array('label' => 'LDAP Group Distinguished Name (e.g. CN=<font color="blue">IEAT-VCD-CI1-Admin</font>,OU=INACC,OU=P001,OU=GRP,OU=Data,DC=ericsson,DC=se )', 'style' => 'width:650px', 'value' => 'CN=PASTEHERE,OU=INACC,OU=P001,OU=GRP,OU=Data,DC=ericsson,DC=se'));
    if (Configure::read('pooling_enabled'))
    {
        ...
        echo $this->Form->input('unrestricted', array('options' => $catalog_details, 'multiple' => true, 'label' => 'Restrict read access to the Catalogs below (select many by holding down Ctrl)',  'size' => '10', 'style' => 'width:650px'));
        echo $this->Form->input('restrict_catalogs', array('label' => 'Restrict read  access to the selected Catalogs above'));
